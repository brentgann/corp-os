...
**Measured at `--repeats 3` the same day (0.25.1).** `corp-os-jobs` 1/3 → **3/3**, `corp-os-decide` 2/3 → **3/3**, and `corp-os-improve`'s packet check 2/3 → **3/3** with `friction_scan.py` in front of it. Four attempts at placement moved two skills from 0/3 to 1/3; one attempt at enforcement took them to 3/3.

`corp-os-glossary` went 1/3 → 2/3 and not further, and the reason was in the new code rather than the skill. **A derived layer is a folder of entries or a single file, and a config that names neither is both until something writes.** `"glossary": {"role": "derived"}` has no `path`, glossary entries go in `glossary.md` at the root, and the check globbed `glossary/*.md` — a folder that does not exist. The gate was silent on precisely the layer shaped that way, which is why the two skills beside it went to 3/3 and this one did not. The harness had this right (`(f"{n}/", f"{n}.md")`) and the enforcement did not, so the two disagreed about what a layer is for one release.

A second defect surfaced only on a fresh OS. `gate_closed_at` makes the failure survive a retry, but until a run closes cleanly there is no such key, and the fallback reference is `usage/log.md`'s mtime — which this very call stamps. So the first OS anyone installs got exactly one enforced gate and then a free pass on the retry. **A failed close now pins the window it used** rather than leaving the next call to re-derive it.

The last of the six failures is not a gate failure at all. `corp-os-improve` opens with *"This skill writes only under `usage/`"* and wrote `claims/pricing.md` in one run of three with that sentence in place — and "write a proposal for `claims/`" is the wrong instruction to hand a run whose actual answer is that the claim should not exist. A declared write scope is checkable at the same step: `--scope-under usage/` fails the close on anything outside it, and the validator fails the build if the declaration and the flag come apart. **A scope stated in prose and not checked anywhere is documentation of an intention.**
...
2. **`recall-load-bearing-sensitive` is a rate, and nobody has measured it.** The check *"the load-bearing sensitive fact reached the answer"* has failed, passed, and failed again across three stored runs of the same case against the same fixture, with the skill unchanged between two of them. CL-0004 is sensitive **and** `load_bearing`: Northwind's champion leaves in March, so an answer computed around it is not cautious, it is wrong in a way nothing in the answer signals. A run that withholds it and a run that reports it are different products, and this suite currently ships whichever one it happens to get.

    §4.25 already treats `corp-os-configure`'s hand-off as "2 times in 5" rather than as a pass or a failure, and `--repeats` exists for exactly this. Nobody has spent it here. **The number is worth more than another wording pass:** three instruction changes were made against `corp-os-configure` before anyone established its rate, and §4.28 records that two of them were aimed at a harness defect. `--repeats 5` on this one case is roughly eight minutes and settles whether this is a coin-flip, a regression, or a fixture that makes the right answer hard to see.

3. **Three skills still have no conformance case:** `audit`, `company`, `contribute`. Each is a fixture problem rather than a harness one. `company` needs a fixture that declares a company layer, `audit` needs one representing someone else's non-corp-os system, and `contribute`'s deliverable leaves the OS entirely, so there is nothing in the tree to assert against without plugin source and a prior diagnosis. The irreversible paths are covered — `rebuild` leaves a `role: source` layer alone in every run, the delete sequence re-cites before destroying, and redaction never touches the original.
4. **The fixtures are three shapes.** The default, a register-profile OS, and one deliberately two releases behind. Still missing: an OS mid-migration with a cohort ceiling being actively worked, and one large enough that the scan contract is doing real work rather than being trivially satisfiable at four entries. Fixture work, not harness work — the runner already takes `--fixture`.
5. **Does the setup interrogation land?** Nine areas, never run on anyone who did not design it. The likely failure is abandonment partway, and no check catches that. A conformance case could at least establish that a run started from nothing produces a valid scaffold, which is weaker but not nothing.
6. **`corp-os-decide` has been tested on one axis.** It survives the roster and one conformance case; whether it fires in a session full of other installed skills is unmeasured.
7. **The `corp-os-configure` hand-off lands 2 times in 5** (§4.25). Not a bookkeeping step, so not scriptable; three instruction passes have not moved it. The next honest move is a person watching a real session take that path, not a fourth wording.
8. **Should `build_index.py` be able to repair, not just report?** It flags undeclared directories, missing `index_line`s and unknown config keys. Auto-fixing would cross the bookkeeping/judgment line that makes it safe to run casually — probably keep as-is.
9. ~~**Does an urgency-tiered layer need ordered rendering?**~~ Done: a layer declares `order_by` and `build_index.py` sorts by that field's declared enum. `now` renders above `watching`. The script reads what the config already said rather than deciding what is urgent, so it stays bookkeeping.
10. **Nothing tells a person their OS is behind.** `corp-os-upgrade` works once someone runs it, and `corp-os-guide` routes "something looks wrong since I updated" to it. Nobody is told unprompted. The cheapest fix is `build_index.py` — it runs constantly and already reads `config.json` — printing a line when the recorded version is behind the plugin's. It was left out of 0.11.0 because the script has no business knowing where the plugin is, and a version check that guesses at a path is worse than none.
11. **Which commands get used?** Whether the eight are *distinguishable* is settled (§4.23). Whether all eight get reached for in practice is a usage question, and `corp-os-improve` should be able to answer it from the log — which now gets written reliably enough to trust.
