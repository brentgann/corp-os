...
def _source_index(root, name, files, spec, cfg, preamble, idx):
    """A source layer's index is a shape, not a listing.

    Measured on a 661-file archive: the general path emitted 661 rows of
    `| [file](file.md) | 0 | — |  |` — 14,943 tokens in which every one of the
    three data columns was structurally empty. Raw files carry no `### `
    entries, no `kind:`/`confidence:`/`status:` mix and no per-file job
    linkage, so the columns a derived layer needs are blank by construction
    rather than by accident. It also read all 661 files to compute them.

    Nothing reads it either. Every skill that wants the queue is pointed at
    the ROOT index, which carries `## Unprocessed queue` separately and
    correctly; nothing in the suite names `raw/INDEX.md` at all.

    What a source layer's index is actually for is orientation: how far back
    the archive goes, how much arrived when, and what is still queued. The
    filename convention (`YYYY-MM-DD--kind--slug.md`) carries period and kind
    already, so the shape is derivable without opening anything — and the two
    or three unprocessed files are worth naming, because those are the only
    ones anybody acts on.
    """
    by_period, kinds, queued = {}, {}, []
    for f in sorted(files):
        base = os.path.basename(f)[:-3]
        m = re.match(r"(\d{4})-(\d{2})-\d{2}--([a-z0-9-]+)--", base, re.I)
        period = f"{m.group(1)}-{m.group(2)}" if m else "undated"
        kind = (m.group(3) if m else "unknown").lower()
        by_period.setdefault(period, {})
        by_period[period][kind] = by_period[period].get(kind, 0) + 1
        kinds[kind] = kinds.get(kind, 0) + 1
        try:                                    # only the frontmatter
            with open(f, encoding="utf-8", errors="replace") as fh:
                head = fh.read(600)
        except OSError:
            continue
        if "processed: true" not in head:
            queued.append((base, kind))

    L = [preamble, "", GEN_MARK, "",
         f"_{len(files)} files · {len(queued)} unprocessed · regenerated "
         f"{date.today().isoformat()}._", "",
         "Source material is never loaded wholesale. This is the shape of the "
         "archive, not\na listing of it — files are named "
         "`YYYY-MM-DD--kind--slug.md`, so anything specific\nis reachable by "
         "path from the entry that cites it.", ""]
    if queued:
        L += ["## Unprocessed", ""]
        L += [f"- [{b}]({b}.md) — {k}" for b, k in sorted(queued)[:60]]
        if len(queued) > 60:
            L += [f"- _… and {len(queued) - 60} more still queued_"]
        L += [""]
    L += ["## By period", "", "| period | files | kinds |", "|---|---|---|"]
    for period in sorted(by_period, reverse=True):
        c = by_period[period]
        L.append(f"| {period} | {sum(c.values())} | "
...
def write_layer_index(root, name, files, spec, cfg, threshold):
    ...
    if spec.get("role") == "source":
        return _source_index(root, name, files, spec, cfg, preamble, idx)
