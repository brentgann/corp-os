...
`capture` is a `config.json` block like any other and its spec lives with the skills it governs: its own file, in `${CLAUDE_PLUGIN_ROOT}/reference/capture.md`. It sets `mode`, `body` and `batch`, and it is what decides a run's cost. `corp-os-pull` and `corp-os-intake` are its only readers and neither reads this file.
